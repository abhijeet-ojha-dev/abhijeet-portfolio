...
// Attach tap-safe touch handling so vertical scroll does not trigger taps on mobile
function addTapSafeTouchEndListener(element, handler) {
    if (!element) return;
    ...
    element.addEventListener('touchmove', (e) => {
        ...
        if (deltaX > CAROUSEL_CONFIG.TAP_MAX_MOVEMENT || deltaY > CAROUSEL_CONFIG.TAP_MAX_MOVEMENT) {
            hasMoved = true;
        }
    }, { passive: true });
...
if (viewCertBtn) {
    ...
    addTapSafeTouchEndListener(viewCertBtn, handleViewCertClick);
}
...
document.addEventListener('keydown', (e) => {
    if (certModal && e.key === 'Escape' && certModal.classList.contains(CSS_CLASSES.ACTIVE)) {
        closeCertModalWithAnimation(certModal);
    }
});
...
if (certImageWrapper) {
    ...
    addTapSafeTouchEndListener(certImageWrapper, handleImageClick);
}
...
if (downloadBtn) {
    ...
    addTapSafeTouchEndListener(downloadBtn, handleDownloadClick);
}
